...
export function checkUsernameAvailability(username, callback) {
  ...
  usernameCheckTimeout = setTimeout(() => {
    if (!username || username.trim().length < 3) {
      callback({ available: false, message: 'Username must be at least 3 characters' });
      return;
    }
...
export function showValidationMessage(inputElement, messageElement, message, isError = true) {
...
export function updatePasswordRequirements(password, requirementsContainer) {
  ...
  const requirementElements = requirementsContainer.querySelectorAll('.requirement');
